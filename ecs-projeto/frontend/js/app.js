// ═══════════════════════════════════════════════════════════════════
// ECS — Front-End Application
// Router, portais, UI, Mentor IA, Matrícula, FAQ
// ═══════════════════════════════════════════════════════════════════
...
var MODALS={
  ...
  'add-turma':{title:'Nova Turma',body:'<div class="fg"><div class="fl">Nome</div><input type="text" class="fi" placeholder="Ex: Blue Team Turma A-2026"/></div><div class="fg"><div class="fl">Trilha</div><select class="fs"><option>Blue Team</option><option>Red Team</option><option>Forense</option></select></div><div style="display:flex;gap:9px;margin-top:14px;justify-content:flex-end"><button class="btn btn-ghost btn-sm" onclick="closeModal()">Cancelar</button><button class="btn btn-cy btn-sm" onclick="showT(\'Turma criada!\',\'s\');closeModal()">Criar</button></div>'},
};
...
function genCert(id){
  ...
  var u=getUser()||{nome:'Aluno'};
  var hash='ECS-'+Math.abs(id*31337).toString(16).toUpperCase().slice(0,8);
  ...
  var body='<div class="w"><p style="font-size:11px;letter-spacing:.3em;color:rgba(0,229,212,.6)">ECS - EDUCA CYBER SYSTEMS</p><h2 style="font-size:10px;color:rgba(255,255,255,.35);margin:5px 0 24px;letter-spacing:.2em">CERTIFICADO DE CONCLUSAO</h2><p style="color:rgba(255,255,255,.45)">Certificamos que</p><div class="n">'+u.nome+'</div><p>concluiu o curso<br><strong>'+(c?c.title:'Curso ECS')+'</strong></p><p style="font-size:11px;color:rgba(255,255,255,.35);margin-top:18px">'+new Date().toLocaleDateString('pt-BR')+'</p><div class="h">Verificacao: '+hash+'</div></div>';
...
function alSupSend(){
  ...
  inp.value='';
  setTimeout(function(){
    msgs.innerHTML+='<div style="align-self:flex-start;background:var(--s3);border:1px solid var(--b1);border-radius:9px;padding:7px 11px;font-size:12.5px;color:var(--t2)">Nossa equipe responderá em breve!</div>';
...
function prAtender(id){ CACHE.update('chamados',id,{status:'resolvido'}); var c=CHAMS.find(function(x){ return x.id===id; }); if(c){ c.status='resolvido'; } renderPrChams(); renderPrDash(); showT('Chamado resolvido!','s'); var sb=initSB(); if(sb) sb.from('chamados').update({status:'resolvido',resolvido_em:new Date().toISOString()}).eq('id',id).catch(function(){}); }
...
function prSaveKB(){
  ...
  var u=getUser();
  sbSalvarKB({problema:p,solucao:s,categoria:cat}, u?u.id:null)
  .then(function(){
...
function initAdmin(){
  ...
  // 1. Carregar do localStorage IMEDIATAMENTE (sem esperar rede)
  loadCacheToArrays();
...
function admApprove(id){
  var u=getUser();
  sbAprovarMatricula(id, u?u.id:null)
  .then(function(){
    renderAdmMats(); renderAdmDash();
    showT('Matricula aprovada!','s');
  });
}
function admReject(id){ CACHE.update('matriculas',id,{status:'cancelada'}); var m=MATS.find(function(x){return x.id===id;}); if(m){m.status='cancelada';} renderAdmMats(); showT('Matricula cancelada','e'); var sb=initSB(); if(sb) sb.from('matriculas').update({status:'cancelada'}).or('id.eq.'+id+',id_local.eq.'+id).catch(function(){}); }
...
function admResolveCham(id){ var c=CHAMS.find(function(x){return x.id===id;}); if(c){c.status='resolvido'; renderAdmChams(); showT('Chamado resolvido!','s');} }
...
function admAddAluno(){
  ...
  var u=getUser();
  sbAdicionarAluno({nome:n,email:e,trilha:t}, u?u.id:null)
  .then(function(data){
    var newUser=data&&data[0]?data[0]:{id:Date.now(),nome:n,email:e};
    ALUNOS.push({id:newUser.id,nome:n,email:e,turma:t,prog:0,status:'ativo',last:'agora',nota:0});
    closeModal(); renderAdmAlunos(); showT('Aluno adicionado no Supabase!','s');
  })
  .catch(function(err){
    ALUNOS.push({id:Date.now(),nome:n,email:e,turma:t,prog:0,status:'ativo',last:'agora',nota:0});
    closeModal(); renderAdmAlunos(); showT('Adicionado (offline)','i');
